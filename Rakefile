...
file "built.iso" => [:isolinux_files, :repodata] do
...
task :repodata => [:yum_configs, "repotrack.done"]
# md5file adds :repotrack as dependency for :repodata only if comps.xml has been changed
# but it currently doesn't work properly
#md5file("os/centos/comps.xml", :repodata, :repotrack)


file "os/centos/comps.xml"

directory "#{BUILD_DIR}/Packages"
file "repotrack.done" => ["os/centos/comps.xml", "#{BUILD_DIR}/Packages"] do
